    ...
    def execute(self, solution):
        ...
        if rand < self.probability:
            ...
            for g in chosen_genes:
                mean_best = np.mean(
                    [
                        self.tracked_algorithm.solutions[j].variables[g]
                        for j in range(self.tracked_best_count)
                    ]
                )
...
class FollowBestMutationSingleGene(Mutation):
    ...
    def execute(self, solution):
        ...
        if rand < self.probability:
            ...
            solution.variables[gene_swap_idx] = self.tracked_algorithm.solutions[
                teacher_idx
            ].variables[gene_swap_idx]
...
class AvoidWorstMutation(Mutation):
    def __init__(self, probability: float, tracked_worst_count: int, avoid_rate: float):
    ...
    def execute(self, solution):
        ...
        to_avoid_idx = n - random.randint(1, self.tracked_worst_count)
        for i in range(len(solution.variables)):
            diff = (
                self.tracked_algorithm.solutions[to_avoid_idx].variables[i]
                - solution.variables[i]
            )
            solution.variables[i] += self.avoid_rate * diff
        return solution

    def get_name(self) -> str:
        return "AvoidWorstMutation"


class AvoidWorstMutationSparse(Mutation):
    def __init__(self, probability: float, tracked_worst_count: int, avoid_rate: float):
        super().__init__(probability)
        self.tracked_worst_count = tracked_worst_count
        self.avoid_rate = avoid_rate
    ...
    def get_name(self) -> str:
        return "AvoidWorstMutationSparse"


class AvoidWorstMutationNorm(Mutation):
    def __init__(self, probability: float, tracked_worst_count: int, norm_fn: callable):
        super().__init__(probability)
        self.tracked_worst_count = tracked_worst_count
    ...
    def execute(self, solution):
        ...
        for i in range(len(solution.variables)):
            ...
            solution.variables[i] += self.norm_fn(diff)
        return solution

    def get_name(self) -> str:
        return "AvoidWorstMutationNorm"


class AvoidWorstMutationMean(Mutation):
    def __init__(self, probability: float, tracked_worst_count: int):
        super().__init__(probability)
        self.tracked_worst_count = tracked_worst_count
    ...
    def execute(self, solution):
        ...
        for i in range(len(solution.variables)):
            ...
            diff = mean_var - solution.variables[i]
            solution.variables[i] += diff
        return solution

    def get_name(self) -> str:
        return "AvoidWorstMutationMean"


class AvoidWorstMutationSingleGene(Mutation):
    pass
